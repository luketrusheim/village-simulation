...
public final class WorldModel {
    ...
    public static void parseDude(WorldModel world, String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == DudeNotFull.DUDE_NUM_PROPERTIES) {
            Entity entity = new DudeNotFull(id, pt, imageStore.getImageList(DudeNotFull.DUDE_KEY), Integer.parseInt(properties[DudeNotFull.DUDE_LIMIT]), Double.parseDouble(properties[DudeNotFull.DUDE_ACTION_PERIOD]), Double.parseDouble(properties[DudeNotFull.DUDE_ANIMATION_PERIOD]));
            world.tryAddEntity(entity);
        }else{
    ...
    public static void parseFairy(WorldModel world, String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Fairy.FAIRY_NUM_PROPERTIES) {
            Entity entity = new Fairy(id, pt, imageStore.getImageList(Fairy.FAIRY_KEY), Double.parseDouble(properties[Fairy.FAIRY_ACTION_PERIOD]), Double.parseDouble(properties[Fairy.FAIRY_ANIMATION_PERIOD]));
            world.tryAddEntity(entity);
        }else{
    ...
    public static void parseTree(WorldModel world, String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Tree.TREE_NUM_PROPERTIES) {
            Entity entity = new Tree(id, pt, imageStore.getImageList(Tree.TREE_KEY), Double.parseDouble(properties[Tree.TREE_ACTION_PERIOD]), Double.parseDouble(properties[Tree.TREE_ANIMATION_PERIOD]), Integer.parseInt(properties[Tree.TREE_HEALTH]));
            world.tryAddEntity(entity);
        }else{
    ...
    public static void parseObstacle(WorldModel world, String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Obstacle.OBSTACLE_NUM_PROPERTIES) {
            Entity entity = new NaturalRiver(id, pt, imageStore.getImageList(Obstacle.OBSTACLE_KEY), Double.parseDouble(properties[Obstacle.OBSTACLE_ANIMATION_PERIOD]));
            world.tryAddEntity(entity);
        }else{
    ...
    public void parseEntity(String line, ImageStore imageStore) {
        ...
        if (properties.length >= ENTITY_NUM_PROPERTIES) {
            ...
            switch (key) {
                ...
                case AbstractDude.DUDE_KEY -> parseDude(this, properties, pt, id, imageStore);
        ...
        while(saveFile.hasNextLine()){
            ...
            if(line.endsWith(":")){
                ...
                switch (line){
                    ...
                    case "Entities:" -> {
                        this.occupancy = new Entity[this.numRows][this.numCols][2];
    ...
    public void load(Scanner saveFile, ImageStore imageStore, Background defaultBackground){
        ...
        if(this.occupancy == null){
            this.occupancy = new Entity[this.numRows][this.numCols][0];
            this.entities = new HashSet<>();
        }
    }

    public void setOccupancyCell(Point pos, Entity entity) {
        this.occupancy[pos.y][pos.x][0] = entity;
    }

    public void setOccupancyCellSecondEntity(Point pos, Entity entity) {
        this.occupancy[pos.y][pos.x][1] = entity;
    }

    public Entity getOccupancyCell(Point pos) {
        return this.occupancy[pos.y][pos.x][0];
    }

    public Entity getOccupancyCellSecondEntity(Point pos) {
        return this.occupancy[pos.y][pos.x][1];
    }

    public Optional<Entity> getSecondOccupant(Point pos) {
        if (isOccupiedSecondCell(pos)) {
            return Optional.of(this.getOccupancyCellSecondEntity(pos));
        } else {
            return Optional.empty();
        }
    }
    ...
    public void removeEntityAt(Point pos) {
        if (withinBounds(pos) && this.getOccupancyCell(pos) != null) {
            ...
            if (this.getSecondOccupant(pos).isPresent()){
                this.setOccupancyCell(pos, this.getSecondOccupant(pos).get());
                this.setOccupancyCellSecondEntity(pos, null);
            }
        }
    }
...
//        if (entity.getKind() == EntityKind.SERPENT){
//            System.out.println("moving guardian");
//        }
        Point oldPos = entity.getPosition();
        if (withinBounds(pos) && !pos.equals(oldPos)) {
            this.setOccupancyCell(oldPos, this.getOccupancyCellSecondEntity(oldPos));
            this.setOccupancyCellSecondEntity(oldPos, null);
            Optional<Entity> occupant = this.getOccupant(pos);

            if (entity.getKind() == EntityKind.SERPENT){
                occupant.ifPresent(target -> this.setOccupancyCellSecondEntity(pos, target));
            } else{
    ...
    public void addEntity(Entity entity) {
        ...
        if (withinBounds(entityPosition)) {

            if (getOccupant(entityPosition).isPresent()){
                this.setOccupancyCellSecondEntity(entityPosition, getOccupant(entityPosition).get());
                this.setOccupancyCell(entityPosition, null);
            }

            this.setOccupancyCell(entityPosition, entity);
            this.entities.add(entity);
        }
    }

    public void addAnotherEntity(Entity entity) {
        Point entityPosition = entity.getPosition();

        if (withinBounds(entityPosition)) {

            this.setOccupancyCellSecondEntity(entityPosition, entity);
            this.entities.add(entity);
        }
    }
    ...
    public boolean isOccupiedSecondCell(Point pos) {
        return withinBounds(pos) && this.getOccupancyCellSecondEntity(pos) != null;
    }
