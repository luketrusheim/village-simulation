...
public final class WorldView {
    ...
    public void drawEntities() {
        List<Entity> serpentList = new ArrayList<>();

        for (Entity entity : this.world.entities) {
            if (entity.getKind() == EntityKind.SERPENT){
                serpentList.addFirst(entity);
            }
        ...
        for (Entity serpent : serpentList){
            Point pos = serpent.getPosition();

            if (this.viewport.contains(pos)) {
                Point viewPoint = this.viewport.worldToViewport(pos.x, pos.y);
                this.screen.image(serpent.getCurrentImage(), viewPoint.x * this.tileWidth, viewPoint.y * this.tileHeight);
            }
        }
    }
